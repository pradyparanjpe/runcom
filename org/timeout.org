...
#+TITLE: Info about current IP
#+SETUPFILE: header.org
#+PROPERTY: header-args :tangle ../bin/timeout.sh :mkdirp t :results no :eval no

* Description
Wait on command, abort if timed out.
Default: print net info within 5 seconds.
=GNU coreutils= provides [[https://www.gnu.org/software/coreutils/manual/html_node/timeout-invocation.html][timeout]];
but we can't assume that it is installed.
