...
*** Bootstrap installation of dependencies
    ...
    For automated setup, only run the [[./setup/dependencies.sh][dependencies script]] with elevated privileges.
    #+BEGIN_SRC sh :tangle no
      curl -sLf "https://pradyparanjpe.github.io/runcom/setup/dependencies.sh" | sudo sh
    #+END_SRC

    #+begin_note
    Inspecting scripts before running them (especially with =sudo=) is generally a good idea.
    The /dependencies/ script is tangled from the code below.
...
* Setup
  ...
  - Copy them elsewhere before proceeding.
...
** Bootstrap setup using script
   - Setup may be bootstrapped by running the [[./setup/setup.sh][setup]] script.
     ...
     #+begin_note
     Inspecting scripts before running them (especially with =sudo=) is generally a good idea.
     The /setup/ script is tangled from the code below.
     If you feel an urge to inspect it or if it fails,
...
* Structure
  ...
  | ${HOME}/.bash_completion.d/*    | no    | no     | $USER |
